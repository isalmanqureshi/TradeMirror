...
from app.api.routes.enrichment import get_trade_context
...
class FakeDB:
    ...
    def scalars(self, stmt):
        ...
        class S:
            def all(self_non):
                return items[: stmt._limit_clause.value] if stmt._limit_clause is not None else items
        return S()
...
def test_missing_market_data_null_safe():
    trade = _trade(exit_minutes=None)
    db = FakeDB([trade])
    context = enrich_trade_context(db, trade.id)
    assert context.atr_percentile is None
    assert context.trend_regime is None
...
def test_batch_summary_counts_and_idempotent():
    ...
    summary2 = batch_enrich_trade_context(db, limit=2)
    assert len(db.contexts) == 2
    assert summary2.enriched == 2
...
def test_get_trade_context_returns_stored_context():
    ...
    out = get_trade_context(trade.id, db)
    assert out.trade_id == trade.id
