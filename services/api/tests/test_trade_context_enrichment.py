...
from app.api.routes.enrichment import enrich_context_batch, enrich_single_trade, get_trade_context
...
class FakeMarketProvider:
    ...
    def get_snapshot(self, symbol, timestamp): return self.snapshot
    def get_atr_percentile(self, symbol, timestamp): return self.atr
    def get_realized_vol_percentile(self, symbol, timestamp): return self.rv
    def get_trend_inputs(self, symbol, timestamp): return self.trend
...
def _trade(hour=14, exit_minutes=24):
    ...
    return Trade(
        id=uuid4(), user_id=uuid4(), strategy_id=None, source_type="live", instrument="futures", symbol="NQ", side="long", entry_time=entry, exit_time=exit_time, entry_price=Decimal("1")
    )


def test_single_trade_enrich_creates_row():
    trade = _trade(); db = FakeDB([trade])
...
def test_session_windows_and_holding_minutes():
    trade = _trade(hour=8, exit_minutes=60); db = FakeDB([trade])
    context = enrich_trade_context(db, trade.id)
...
def test_atr_mapping_and_trend_mapping():
    trade = _trade(); db = FakeDB([trade])
    provider = FakeMarketProvider(atr=50, rv=85, trend={"close": 100, "ma_50": 100, "ma_50_slope": 0})
...
def test_missing_market_data_null_safe_and_marked():
    trade = _trade(exit_minutes=None); db = FakeDB([trade])
    context = enrich_trade_context(db, trade.id)
...
def test_event_proximity():
    ...
    assert context.macro_event_nearby is True and context.macro_event_name == "CPI" and context.minutes_to_event == 30


def test_batch_summary_counts_and_idempotent():
    t1, t2 = _trade(), _trade(hour=22); db = FakeDB([t1, t2])
    summary = batch_enrich_trade_context(db, limit=2)
    assert summary.processed == 2 and summary.enriched == 2
    summary2 = batch_enrich_trade_context(db, limit=2, skip_existing=True)
    assert len(db.contexts) == 2 and summary2.skipped == 2
...
def test_routes_use_injected_providers():
    ...
    summary = enrich_context_batch(db=db, market_data_provider=provider, event_provider=event_provider)
